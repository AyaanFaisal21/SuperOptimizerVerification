...
| 1.5 | The "100x tolerance gap" claim. Compared Axon's stated threshold (AWS/UIUC) against MPK runtime-kernel tests (CMU): different group, subsystem, and artifact. Mirage's paper states no float tolerance; its superoptimizer path has no float check at all | A public critique of named researchers, built on a category error | An external request to verify, not an internal check | A number that supports the thesis deserves more scrutiny, not less |
